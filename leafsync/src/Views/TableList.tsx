...
export default function TableList() {
  ...
  useEffect(() => {
    const fetchTables = async () => {
      try {
        const response = await axios.post("http://localhost:5000/gettable", {
          hostname: "localhost",
          password: "",
          username: "root",
  ...
  return (
    <div className="min-w-[8rem] flex flex-col text-white">
        ...
        <div>
            ...
            <p className="cursor-pointer text-[.8rem] text-blue-400 capitalize pt-1 pb-1 pl-2 pr-2 overflow-x-hidden" key={index}>{table}</p>
            ))}
        </div>
    </div>
  );
